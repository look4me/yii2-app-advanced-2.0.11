...
class ChuaishuxiongController extends \yii\web\Controller
{
    public function actionTurntable()
    {
        ...
        if (!isset($_COOKIE['deviceid4'])) {
            $deviceid = md5(time() . rand(100000, 9999999));
            setCookie('deviceid4', $deviceid, NULL, '/');
            $user = new XUser();
        ...
        if (!isset($_COOKIE['num4'])) {
            $num = 8;
            $time = strtotime(date('Y-m-d')) + 24 * 3600;
            setCookie('num4', $num, $time, '/');
        } else {
            $num = $_COOKIE['num4'];
        }
        $testkey = 'none';
...
//        echo ($channel);
        return $this->render('chuaishuxiong',
            array(
                'num' => $num,
                'channel' => $channel,
                'testkey' => $testkey,
    ...
    {
        $url = Yii::app()->createUrl('chuaishuxiong/turntable');
        Header("Location: $url");
    }
