...
class Lottery2Controller extends \yii\web\Controller
{
    public $layout = false;
    ...
    public function actionTurntable()
    {
        ...
        if (!isset($_COOKIE['deviceid2'])) {
            $deviceid = md5(time() . rand(100000, 9999999));
            setCookie('deviceid2', $deviceid, NULL, '/');
            $user = new XUser();
        ...
        if (!isset($_COOKIE['num2'])) {
            ...
            setCookie('num2', $num, $time, '/');
        } else {
//            var_dump(setCookie('num2',1,NULL,'/'));
//            echo $_COOKIE['num2'];die;
            $num = $_COOKIE['num2'];
        }
        $testkey = 'none';
        if ($num <= 2) {
    ...
    {
        $url = Yii::app()->createUrl('lottery2/turntable');
        Header("Location: $url");
    }

    public function actionPrize()
    {
        $deviceid = $_COOKIE['deviceid2'];
        $num = $_COOKIE['num2'];
        if ($num < 0) {
            $arr = array(
                'code' => 1,
